...
import fetchSaleTypeWiseProduct from '../helpers/fetchSaleTypeWiseProduct';


const FlashSales = ({saleType, heading}) => {
    ...
    const fetchData = async() =>{
        setLoading(true)
        const saleTypeProduct = await fetchSaleTypeWiseProduct(saleType)
        setLoading(false)

        console.log("horizontal data",saleTypeProduct.data)
        setData(saleTypeProduct?.data)
    }
  ...
  return (
    <div className='container mx-auto bg-white px-8 my-4 py-2 rounded-lg relative'>
           ...
           <div className='flex items-center gap-2  md:gap-1 md:flex-wrap  overflow-x-scroll scrollbar-none transition-all' ref={scrollElement}>
           ...
           
           {
                ...
                loading ? (
                    loadingList.map((product,index)=>{
                        return(
                            <div className='w-full min-w-[115px]  md:min-w-[199px] max-w-[200px] md:max-w-[140px]  bg-white rounded-lg shadow '>
                                <div className='bg-slate-200 h-16 md:h-24 p-4 rounded-md min-w-[115px] md:min-w-[145px] flex justify-center items-center animate-pulse'>
                                </div>
                    ...
                    data.map((product,index)=>{
                        return(
                            <Link to={"product/" + product?._id} className='w-full mb-2 rounded-md min-w-[115px] relative md:min-w-[199px] max-w-[200px] md:max-w-[160px] bg-white shadow hover:scale-100 hover:rounded-md transition-all'>
                           <div className='absolute top-2 left-4 md:top-4 md:eft-5 text-xs md:block' onClick={(e) => handleAddToWishList(e, product?._id)}>
                            ...
                            <div className=' w-full bg-slate-200 h-16 md:h-24 p-4 rounded-md min-w-[115px] md:min-w-[160px] flex justify-center items-center'>
